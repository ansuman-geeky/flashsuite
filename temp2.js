      ...
      gtag('config', 'G-3B61CKGJL7');
        ...
        function convertPdfTextToFabric(span, page) {
            ...
            // Math guarantee: Calculate exact font size by matching the visible string width
            const dummyBox = new fabric.Textbox(textStr, { fontSize: 100, fontFamily: fontFamily, fontWeight: fontWeightStr, fontStyle: fontStyleStr });
            const fabricBaseWidth = dummyBox.width;
            let exactFontSize = 12;
            if (fabricBaseWidth > 0 && relativeWidth > 0) {
                exactFontSize = 100 * (relativeWidth / fabricBaseWidth);
            } else {
                exactFontSize = relativeHeight * 0.75;
            }
            ...
            // Lock scroll brute-force
            const wsCenter = document.querySelector(".workspace-center");
            const lockedScrollTop = wsCenter ? wsCenter.scrollTop : 0;
            const lockedScrollLeft = wsCenter ? wsCenter.scrollLeft : 0;

            setTimeout(() => {
                fCanvas.setActiveObject(editableTextbox);
                editableTextbox.enterEditing();
                
                // Hack: Patch the created hidden textarea immediately to prevent browser scroll jump
                if (fCanvas.hiddenTextarea) {
                    fCanvas.hiddenTextarea.focus = function() { HTMLElement.prototype.focus.call(this, { preventScroll: true }); };
                }
                
                if (wsCenter) {
                    wsCenter.scrollTop = lockedScrollTop;
                    wsCenter.scrollLeft = lockedScrollLeft;
                }
                
                editableTextbox.selectAll();
                fCanvas.renderAll();
                
                // Keep enforcing scroll lock for 50ms while browser handles focus events
                if (wsCenter) {
                    let lockInterval = setInterval(() => {
                        wsCenter.scrollTop = lockedScrollTop;
                        wsCenter.scrollLeft = lockedScrollLeft;
                    }, 5);
                    setTimeout(() => clearInterval(lockInterval), 50);
                }
            }, 50);
