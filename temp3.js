      ...
      gtag('config', 'G-3B61CKGJL7');
        ...
        function convertPdfTextToFabric(span, page) {
            ...
            const fontWeightStr = computedStyle.fontWeight === "bold" || parseInt(computedStyle.fontWeight) >= 700 ? "bold" : "normal";
            const fontStyleStr = computedStyle.fontStyle === "italic" ? "italic" : "normal";

            // True Font Size Extraction: PDF.js sets inline font-size. We MUST use that to prevent ballooning.
            let exactFontSize = 12;
            let rawInlineSize = span.style.fontSize ? parseFloat(span.style.fontSize) : NaN;
            if (!isNaN(rawInlineSize) && rawInlineSize > 0) {
                exactFontSize = rawInlineSize; // PDF.js inline sizes are the ultimate source of truth
            } else {
                let compSize = parseFloat(computedStyle.fontSize);
                if (!isNaN(compSize) && compSize > 0) {
                    exactFontSize = compSize / zoomScale;
                } else {
                    exactFontSize = relativeHeight * 0.65; // Highly conservative fallback
                }
            }

            // Add safety bounds
            if (exactFontSize < 4) exactFontSize = 4;
            if (exactFontSize > 150) exactFontSize = 150;

            // Extract CSS Transform to preserve stretched PDF fonts
            let cssScaleX = 1, cssScaleY = 1;
            if (computedStyle.transform && computedStyle.transform !== "none") {
                const m = computedStyle.transform.match(/matrix\(([^,]+),\s*([^,]+),\s*([^,]+),\s*([^,]+)/);
                if (m) {
                    let px = parseFloat(m[1]), py = parseFloat(m[4]);
                    if (px > 0 && isFinite(px)) cssScaleX = px;
                    if (py > 0 && isFinite(py)) cssScaleY = py;
                }
            }

            // Adjust width to compensate for scaleX so it doesn't reflow prematurely
            const naturalWidth = (relativeWidth / cssScaleX) + 10;
            ...
            setTimeout(() => {
                ...
                // ROOT CAUSE FIX: Fabric JS places the hidden textarea at top:0, left:0 of the canvas.
                // When focused, the browser aggressively scrolls to top:0. We MUST move the textarea
                // to the exact pixel where the user clicked to completely nullify the scroll jump!
                if (fCanvas.hiddenTextarea) {
                    fCanvas.hiddenTextarea.style.top = relativeTop + "px";
                    fCanvas.hiddenTextarea.style.left = relativeLeft + "px";
                    fCanvas.hiddenTextarea.focus = function() { HTMLElement.prototype.focus.call(this, { preventScroll: true }); };
                }
